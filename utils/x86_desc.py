...
class SegmentDescriptor(Descriptor32):
    ...
    def hdr(self):
        fmt = "%-6s | %-10s | %-7s | %-10s | %-13s | %-3s | %s | %s | %s | %s\n"
        hdr = fmt % ("Select","Base","Limit","Size","Type","DPL","P","L","D","G")
        return self.bar()+hdr+self.bar()

    def __str__(self):
        return self.hdr()+"(none)"+repr(self)+self.bar()
...
class InterruptDescriptor(Descriptor32):
    ...
    def hdr(self):
        fmt = "%-6s | %-6s | %-10s | %-13s | %-3s | %s\n"
        hdr = fmt % ("Index","Select","Offset","Type","DPL","P")
        return self.bar()+hdr+self.bar()
